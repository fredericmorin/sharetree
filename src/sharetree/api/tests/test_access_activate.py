"""Tests for POST /api/v1/access_code/{code}/activate."""
...
def activate_url(code: str) -> str:
    return f"/api/v1/access_code/{code}/activate"
...
def test_activate_unclaimed_code_succeeds(client):
    with (
        ...
        res = client.post(activate_url("abc123"))
...
def test_activate_claimed_code_returns_404(client):
    with (
        ...
        res = client.post(activate_url("claimed"))
...
def test_activate_nonexistent_code_returns_404(client):
    with (
        ...
        res = client.post(activate_url("doesnotexist"))
...
def test_activate_claimed_code_does_not_call_set_session(client):
    with (
        ...
        client.post(activate_url("claimed"))
...
def test_activate_records_session_id_on_first_claim(client):
    with (
        ...
        client.post(activate_url("newcode"))
